...
function NewProperty(props) {
	...
	const handlePriceChange = (e) => {
		// (e) => {(e.target.value > 9999999999) ? setMv(mv) : setMv(e.target.value); calculateActualPrice(e.target.value, disPer); RupeeConversion(e.target.value)}
	...
	const changeStep = (step, type) => {
		...
		if(type === 'back'){
			...
			alert.error(<div style={{color:'#FF0040'}}>{stepHead[step-2] + ' is invalid'}</div>);
		}
	}
	...
	const RupeeConversion = (value) => {
		...
		if (count == 6) {
			...
			setMvWords(first + "." + last + " Lacks");
		} else if (count == 7) {
			second = strNumber[1];
			last = strNumber.slice(2, 4);
			setMvWords(first + second + "." + last + " Lacks");
		} else if (count == 8) {
			last = strNumber.slice(1, 3);
			...
			setMvWords(first + second + third + "." + last + " Crore");
		} else if (count == 11) {
			second = strNumber[1];
			var third = strNumber[2]
			last = strNumber.slice(4, 6);
			setMvWords(first + second + third + strNumber[3] + "." + last + " Crore");
		} else if (count == 12) {
			second = strNumber[1];
			var third = strNumber[2]
			last = strNumber.slice(6, 8);
			setMvWords(first + second + third + strNumber[4] + strNumber[5] + "." + last + " Crore ");
		} else if (count == 13) {
			second = strNumber[1];
			// var third = strNumber[2]
			last = strNumber.slice(2, 4);
			setMvWords(first + "." + last + " Lakh Crore ");
		} else if (count == 14) {
			second = strNumber[1];
			var third = strNumber[2]
			last = strNumber.slice(3, 5);
			setMvWords(first + second + "." + last + " Lakh Crore ");
		}
	}
	...
	const postProperty = async () => {
		...
		console.log({step, pFor, propAge, type, subCat, exCat, loc, yr, st, ct, otherAddress, ar, arUnit, mv, disPer, photo: photoArray});
		try {
							...
							{ step == 6 && 
								<div className="step7">
									...
									<div className="row">
										...
										<div className="col-6 value-in-txt">
											After discount Price &#8377; &nbsp;{actualPrice}
											{/* <p className="mb-1">2 Crore 45 lacks</p> */}
										</div>
									</div>
									<div className="row">
										<div className="col-12">
											<input type="number" placeholder="Enter Price in Rupee" value={mv} onChange={handlePriceChange}  className="w-100 value-area-input"/>
				...
				<div className="container">
					...
					<div className="col-md-7 align-self-center">
						...
						<div className="col-8">
							...
							<div className="col-12 text-center pb-4">
								<p className="pay-only-txt m-0">Pay only 499/-</p>
								<p className="pay-only-txt m-0">to post a Property</p>
							</div>
							<div className="col-12 text-center pb-2">
								<a href="#" className="btn-pay-post" disabled={submitDisabled} onClick={() => postProperty()}>{submitDisabled ? 'Wait...' : 'Pay & Post' }</a>
							</div>
							{/* <div className="col-12 text-center pb-2">
